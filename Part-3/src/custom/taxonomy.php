<?php
/**
 * Custom Taxonomy Handler - this code generator handles building the
 * labels, arguments, and then registering the taxonomy with WordPress.
 *
...
function register_the_custom_taxonomies() {
	$config = apply_filters( 'get_custom_taxonomy_runtime_config', array() );

	foreach ( $config as $taxonomy => $config ) {
...
function register_the_custom_taxonomy( $taxonomy, array $config ) {
	...
	if ( ! $args['labels'] ) {
		$args['labels'] = generate_the_custom_labels( $taxonomy, $config['labels'] );
	}
